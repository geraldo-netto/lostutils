...
oze-rel-01 | open | low | organize_by_extension.py:984 — move_file/_link_exclusive never verify the source is a regular file before os.link; only the planning-collision path (_resolve_one_planning_collision, 1574) does the lstat+S_ISREG check. plan_moves/move_file are public and can be handed a symlink source, which then gets hardlinked into a bucket — relocating link semantics the scanner deliberately refuses to follow. Add an S_ISREG guard in move_file. | public move entrypoint skips the symlink check
...
oze-robust-02 | open | med | organize_by_extension.py:1277-1289 — _move_cross_device's `except BaseException` cleanup unconditionally os.unlink(target) even after os.replace already committed the file; a KeyboardInterrupt fired after a successful os.replace but before the try exits deletes the freshly-completed target while source removal (1290) never ran — destroying just-done work and dropping the reservation. Set a committed flag after os.replace and skip unlinking target once committed. | Ctrl-C interrupt safety, EXDEV path
oze-robust-01 | open | med | organize_by_extension.py:1290 — _move_cross_device runs os.unlink(source) after os.replace with no rollback; if that unlink fails (EACCES/EBUSY/EROFS) the file exists at BOTH paths as full independent copies (cross-fs is a real copy), disk doubled and source orphaned, surfaced only as a bare OSError. The same-fs path has _unlink_with_rollback; this path has none. Wrap the unlink and log "moved but source not removed — duplicate left at <source>". | asymmetric with hardlink path's rollback
...
oze-obs-02 | open | med | organize_by_extension.py:1621 — _drain_futures calls fut.result(), which re-raises any exception the worker did NOT catch (_move_worker only traps OSError/RuntimeError/ValueError); an unexpected class (KeyError/MemoryError) propagates out of _run_moves, bypassing the per-file skip path, aborting the whole batch and losing all in-flight progress with no summary. Wrap fut.result() to log the offending source and decide skip-vs-fatal. | one rogue file kills the batch
