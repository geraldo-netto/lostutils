...
Latest verification (after the CI-abort fix batch): `ruff@0.15.22 check *.py` → all checks passed; `pyright@1.1.411 --pythonpath /usr/bin/python3.12 *.py` → 0 errors/warnings/informations; `lizard -l python -C 11 *.py` → 0 warnings over 1,374 functions; CI-equivalent suite 2,282 passed / 1 skipped / 6 subtests / **0 warnings** in 112s plus 103 explicitly collected fuzz cases; total coverage 97.64%; all 1,372 functions/methods meet the 80% function gate. The Python 3.12 POSIX-fork warning is gone — `bt-mt-01` shipped, and the only remaining `fork` use is an explicit, locally-suppressed test seam.

CI was red from `8ce98c79` to `8ac6a11` — every push aborted with SIGABRT (exit 134) partway through the suite, at a point that moved between runs. Three defects, found by reading the core dump `systemd-coredump` had kept (`Tcl_Panic` ← `Tcl_AsyncDelete`, on a non-main thread), all fixed and each committed on its own. `mkp-mt-01`: a Tk root is inside a reference cycle, so `destroy()` leaves the `Tkapp` for the cyclic collector, and Tcl aborts the process when a thread other than the interpreter's creator finalizes it — the Tk-using test modules leaked roots into the later thread-pool tests, which supplied exactly such a thread. Both now collect on the main thread after each test, which also cleared 18 `PytestUnraisableExceptionWarning`s the leak had been emitting. `oze-time-01`: `_drain_futures` bound `time.monotonic` as a default argument, so the move watchdog was frozen on the real clock and a substituted one could never reach the abort deadline — the stall test burned 300 real seconds emitting ~7.5M warning lines into pytest's capture, which is what exhausted memory. The suite went from 443s to 112s. `ie-rel-01`: `_merge_text_blocks` cut at the character budget without stripping, so a cut landing mid-line kept a separator the normalizer had already collapsed and re-merging changed the text — a latent non-idempotence that only surfaced when Hypothesis happened to generate the case, i.e. a red build on a random future run.
...
Second multiplatform rescan: 2026-08-01 — the same 10 root scripts swept again with a different reviewer, reading each file end to end rather than grepping for known API families. It found 22 further defects, none duplicating the first sweep's fixes or its four rejections, and all 22 have since shipped (one commit each — see `git log`), so the `platform` table is empty again. The worst of them were silent: `hr-plat-06` (hash-recursive reported zero duplicates on any tree on Windows, exit 0, because `DirEntry.stat` gives `st_dev`/`st_ino` as 0 there and the whole tree folded into one inode), `rdv3-plat-03` (a case-variant path pair on macOS/Windows made the emitted script delete the only copy), `lq-plat-06` (every shipped default command failed on a stock Windows install, since `echo` is a cmd.exe builtin and the protocols run `shell=False`), `oze-plat-05` and `oze-plat-04` (organize skipped every file on FAT/exFAT, or permanently re-skipped case-variant names, off Linux). Two — `ie-plat-03` and `lq-plat-05` — were defects inside the first sweep's own `OpenProcess` fix, in both copies of the probe the standalone-script rule requires. `dedupl_numpy.py` and `deduplicate-by-namev3.py` came back clean.

Three fixes there are worth knowing about because they changed behaviour rather than just repairing it. `oze-plat-04` decides case-folding by probing the destination filesystem (one temp file per run, cached, with `reset_case_fold_cache()` as the reset hook) rather than by `os.path.normcase`, which is identity on macOS where APFS folds anyway; folding stays off on case-sensitive hosts so existing layouts do not move. `rdv3-plat-03` now withholds case-variant paths from removal entirely and emits a comment naming them — on ext4 those really are two files, so this trades a missed dedup for never deleting a unique file, which is the right way round for a tool whose only job is nominating deletions. `mkp-plat-03` gives every key a `highlightbackground` ring because native Aqua renders neither `bg` nor `relief` on a `tk.Button` face; that restores all three key states on macOS, not just the selected one.
...
mkp-ui-01 | open | low | minikeypad.py:1133 — the fixed-size main window has no position, so launch placement depends on the window manager and can appear away from the user's focus. Center the window on screen with edge clamping. | Laws of UX — Aesthetic-Usability Effect; deterministic balanced initial placement
