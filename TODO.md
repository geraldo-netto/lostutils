...
Latest verification (after the second multiplatform fix batch): `ruff@0.15.22 check *.py` → all checks passed; `pyright@1.1.411 --pythonpath /usr/bin/python3.12 *.py` → 0 errors/warnings/informations; `lizard -l python -C 11 *.py` → 0 warnings over 1,374 functions; CI-equivalent suite 2,280 passed / 1 skipped / 6 subtests plus 103 explicitly collected fuzz cases; total coverage 97.64%; all 1,372 functions/methods meet the 80% function gate. The Python 3.12 POSIX-fork warning is gone — `bt-mt-01` shipped, and the only remaining `fork` use is an explicit, locally-suppressed test seam.
