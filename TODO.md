...
Latest targeted rescan: 2026-08-01 — multiplatform sweep of all 10 root scripts against the new AGENTS.md "multiplatform by design" rule. Checked POSIX-only module/API use, hardcoded POSIX paths, `os.rename`/`os.link`/`chmod` divergence, subprocess and signal portability, path separators, home/config discovery, and filename legality. Findings are in the `platform` table; two candidates were verified as non-defects and recorded under "Audit picks deliberately rejected".
...
oze-plat-01 | open | low | organize_by_extension.py:1447 — `_reserve_slot_via_rename` calls `os.rename(source, candidate)` onto the placeholder it just created with `O_CREAT\|O_EXCL`; POSIX rename replaces an existing target but Windows `os.rename` raises `FileExistsError`, so the hardlink-free fallback path can never reserve a collision slot there. Use `os.replace`, which has the POSIX semantics on both. | platform: documented Windows divergence in `os.rename`
oze-plat-02 | open | low | organize_by_extension.py:1594-1598 — `_link_regular_no_follow` retries `os.link(src, dst)` only on `TypeError`, but a platform that lacks `follow_symlinks` support (Windows) raises `NotImplementedError` from `os.link`, which escapes as a hard failure instead of falling back. Catch both. | platform: `os.supports_follow_symlinks` excludes `os.link` on Windows
oze-plat-03 | open | low | organize_by_extension.py:83-94 — `normalize_extension` returns the raw lowercased suffix as the bucket directory name, so a `.aux` or `.prn` file asks for a directory named `aux`/`prn`; those are reserved DOS device names and `mkdir` fails on Windows for the whole extension. Map the reserved names to a non-reserved spelling on Windows only, so existing POSIX trees keep their layout. | platform: Windows reserved device names (`CON`, `PRN`, `AUX`, `NUL`, `COM1-9`, `LPT1-9`)
rf-plat-01 | open | low | relocate_folder.py:2817 — `main` runs the whole plan before reaching the POSIX-only primitives it depends on (`os.chown`, `os.symlink`, `/proc` scanning), so on Windows it fails partway with a bare `AttributeError: module 'os' has no attribute 'chown'` instead of a clear refusal. The script is legitimately POSIX-specific (per AGENTS.md), so it should say so and exit cleanly before touching anything. | platform: the AGENTS.md multiplatform rule's "degrade gracefully — never crash" clause for a genuinely OS-specific script
