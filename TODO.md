...
| mkp-test-61 | open | low | low | minikeypad.py:932 — [test coverage] Add focused coverage for ProfileStore.save: the full suite plus fuzz reaches 75% statement coverage, below the enforced 80% per-function gate. Verify failed profile saves preserve the existing file and clean temporary resources. |
| oze-test-61 | open | low | low | organize_by_extension.py:1771 — [test coverage] Add focused coverage for _open_pinned_regular: the full suite plus fuzz reaches 78.57% statement coverage, below the enforced 80% per-function gate. Verify changed or replaced files are rejected without leaking descriptors. |
