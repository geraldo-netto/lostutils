...
id | status | effort | description | notes
--- | --- | --- | --- | ---
bt-robust-01 | open | high | bookmark-tidy.py:377 — `sqlite3.connect(f"file:{source}?mode=ro", uri=True)` (also :389) builds a `file:` URI by raw string interpolation of a filesystem path; a Windows path (`C:\Users\...`) or any path with `?`, `#`, or `%` produces a malformed/misparsed URI, so Firefox reads silently fail or open the wrong file. Encode with `urllib.request.pathname2url` (or `Path.as_uri()`) before appending `?mode=ro`. | robustness/recovery + platform — Windows portability
