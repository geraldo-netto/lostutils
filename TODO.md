...
id | status | effort | description | notes
--- | --- | --- | --- | ---
hr-conc-02 | open | low | hash-recursive-ai5.py:447-454 — the FileNotFoundError branch increments hash_skipped_vanished, but a file deleted mid-read (after open) raises OSError inside _read_window_into, not FileNotFoundError on open, so it is counted as a real hash error (455) not a vanished skip; the vanished/real split is only partially accurate. Classify ENOENT/ESTALE from the read path into the vanished counter too. |
hr-conc-01 | open | med | hash-recursive-ai5.py:604 — the SIGINT cancel_event is only checked by walk workers (198); _run_stage and the hash batch closures never consult it, so a Ctrl-C during a long stage-1/stage-2 hash phase has no effect until a second Ctrl-C. Thread cancel_event into _run_stage and check it between batches. | UX
lq-conc-02 | open | low | link_queue.py:1069-1074 — _dispatch_immediate prunes only dead threads, but _run_immediate_item blocks on the semaphore (1047) before running, so a large paste keeps all not-yet-started runner threads alive and the list grows to the paste size before draining. Use a real bounded immediate pool. | memory
lq-conc-01 | open | low | link_queue.py:4128-4134 — shutdown saves state BEFORE stop_event.set(), then cancels the debounced timer; workers can still finish/claim items between the save and the join, mutating queue_items/current_items, and those final transitions are never persisted. Snapshot AFTER the worker join (or save once more post-join). | shutdown correctness
...
id | status | effort | description | notes
--- | --- | --- | --- | ---
hr-cx-01 | open | low | hash-recursive-ai5.py:957 — `config.alias_cap < 2**31` re-implements the "is the cap disabled" sentinel that RunConfig.__init__ (111) encapsulates as the magic 2**31; the sentinel is duplicated in two places. Expose a property like config.alias_cap_active or a named NO_CAP constant. | DRY
...
lq-cmplx-01 | open | med | link_queue.py:2477-2535 — LinkQueueApp delegates to Dispatcher via BOTH __getattr__ (2477) and ~25 static method re-exports (2511-2535); the dual mechanism is redundant and a method added to Dispatcher silently won't appear in the re-export list. Consolidate on one delegation path. | SOLID
...
hr-rel-01 | open | low | hash-recursive-ai5.py:687 — index_inodes does inode_size[key] = size (last-writer-wins) for every alias; if two walk entries for the same (dev,ino) report different sizes (stat race) the inode silently inherits the last size, which drives CAP/2*CAP gating and stage-2 sample offsets. Use inode_size.setdefault(key, size) or assert equality. | correctness
hr-rel-02 | open | med | hash-recursive-ai5.py:1196 — find_duplicate_groups is driven by the lazy walk_iter generator; if hashing raises, the generator is abandoned mid-iteration, the walk daemon threads are never joined and keep scandir-ing, and walk_iter.stats is never finalized (309-310 runs only on normal completion) so the except path reads stale zeroed stats. Drive the walk to completion or join the coordinator in finally. | correctness
hr-rel-03 | open | low | hash-recursive-ai5.py:1204 — the `walk_boundary[0] is None` guard is documented as firing only if find_duplicate_groups raises before index_inodes returns, but on_walk_done fires after index_inodes consumes the whole iterator (961-962); the guard is dead for the documented case. Move on_walk_done earlier or drop the misleading comment. |
...
lq-rel-01 | open | low | link_queue.py:475-481 — _merge_user_config: a user config with `protocols:` set to a non-dict (null/list/scalar) fails the isinstance dict guard, falls into else: cfg[k]=v, and overwrites the default dict; _normalize_config_schema then calls cfg["protocols"].items() (491) and raises AttributeError, crashing startup. Skip a non-dict protocols in else, or coerce to {} before iterating. | corrupt config
lq-rel-02 | open | med | link_queue.py:716 — _immediate_slots semaphore is sized once at construction from worker_count and never resized by _ensure_worker_count/_on_worker_count_changed; raising Workers leaves immediate-mode throttled to the original count. Rebuild the semaphore (or use a resizable counter) when worker_count changes. | live config
lq-rel-05 | open | low | link_queue.py:746-762 — _request_save_state re-arms the timer; between shutdown's stop_event.set() (4130) and _cancel_save_timer() (4134) a worker-step _request_save_state (1820) can arm a timer that fires _flush_save_state post-teardown. Re-check stop_event inside _flush_save_state before writing. | shutdown race
lq-rel-04 | open | low | link_queue.py:981-991 — _batch_dispatch mutates _batch_dispatch_depth/_batch_save_dirty outside any lock, but the depth is read under _dispatch_cv in _enqueue_or_skip_duplicate (1085) and _persist_after_enqueue (1110); a concurrent enqueue can read a torn depth and double-notify or skip the on-exit notify. Guard depth/dirty under _dispatch_cv. | data race
lq-rel-03 | open | low | link_queue.py:4047 — _on_queue_rerun calls _process_link(it.url) and drops it.extra, so re-running a queued item silently loses its mapped flags (e.g. -o clip.mp4). Pass it.extra: self._process_link(it.url, it.extra). | data loss
