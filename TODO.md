...
id | status | effort | description | notes
--- | --- | --- | --- | ---
bt-plat-01 | open | low | bookmark-tidy.py:251 — `_read_utf8_text` reads with `encoding="utf-8"` rather than `"utf-8-sig"`, so a UTF-8 BOM survives as `U+FEFF`; `str.lstrip()` does not strip it (it is not Python whitespace), so the format sniff at 606-612 matches neither `<` nor `{`. | platform: a bookmark file round-tripped through Windows tooling (Notepad, `Out-File -Encoding utf8`) raises `UserError("unsupported bookmark file")` and `read_all_bookmarks` (1586) downgrades that to a warning and drops the file from the merge — a valid input silently excluded. `utf-8-sig` is identical for BOM-less input, so the fix has no downside. Confidence med: mechanism verified, triggers only on BOM'd files
bt-plat-02 | open | low | bookmark-tidy.py:655 — `_supported_input_file` compares `path.name == "Bookmarks"` case-sensitively, while the suffix check on the very same line already uses `.casefold()`. | platform: on Windows/macOS an explicitly named Chrome profile file typed as `...\Default\bookmarks` passes the `exists()` check (669) on the case-insensitive filesystem, then `_expand_one_input` returns `[]` (677) and the run fails with the misleading `no bookmark inputs found` (1753). `Places.sqlite` is rescued by the casefolded `.sqlite` suffix; only the extension-less name is affected. Confidence med
...
ie-plat-03 | open | low | import_events.py:4159 — `_lock_owner_is_running_windows` reads `kernel32.GetLastError()` through a `ctypes.windll` handle not created with `use_last_error=True`, which ctypes documents as unreliable because interpreter/ctypes calls between `OpenProcess` and the separate FFI call can clobber the thread's LastError. | platform: defect introduced by the ie-plat-01 fix itself, not a re-report of it. Windows only: when `OpenProcess` fails with ERROR_ACCESS_DENIED for a live owner under another user or elevation, a clobbered LastError returns False, `_lock_is_stale` (4199) declares the live owner dead, the lock is reclaimed (4226-4234), and two runs write the same output — exactly the race the lock exists to prevent. Fix: `ctypes.WinDLL("kernel32", use_last_error=True)` plus `ctypes.get_last_error()`. Confidence med
...
ie-plat-05 | open | low | import_events.py:4676 — the summary loop prints untrusted event titles and locations to `sys.stdout` with no encoding-error handling, and the module never calls `sys.stdout.reconfigure` (grep-verified: no `reconfigure` anywhere in the file). | platform: on Windows with stdout redirected (`python import_events.py > run.log`) the stream uses the locale code page with `errors="strict"`, so a CJK/Cyrillic/Greek/Hebrew title — scripts this tool's own language detection at 225-232 advertises — raises `UnicodeEncodeError`. Nothing between 4676 and `main` catches it (only `KeyboardInterrupt` and `ModelUnavailableError` are), so the process dies with a traceback, skips the epilogue at 4778-4789 and breaks the documented 0/1/2 exit contract, even though the JSON/ICS output was already written. Confidence high
ie-plat-06 | open | low | import_events.py:4798 — `_APP_LOG_FILE = os.fdopen(os.dup(2), "w", buffering=1)` re-wraps stderr's fd with no `encoding=`/`errors=`, dropping the `errors="backslashreplace"` safety the original `sys.stderr` wrapper carries. | platform: on Windows the wrapper defaults to the ANSI code page with `errors="strict"` (`os.fdopen` never takes the UTF-8 `_WindowsConsoleIO` path, so this applies to console and redirected stderr alike). Any record holding a character outside that code page — untrusted filenames, the LLM excerpt from `_llm_response_log_context` (2302), OCR text — raises `UnicodeEncodeError` inside `StreamHandler.emit`, which `handleError` swallows: the record is dropped and replaced by `--- Logging error ---`. Confidence high
...
lq-plat-08 | open | low | link_queue.py:2416 — both `subprocess.Popen` calls open the child's stdout with `text=True` and no `encoding=` (exec branch identical at 2453), so decoding uses the locale default. | platform: on Windows that is the ANSI code page (typically cp1252) while the tools this app targets (yt-dlp, aria2c, curl) emit UTF-8. Non-ASCII output is mojibake at best; a UTF-8 sequence containing 0x81/0x8D/0x8F/0x90/0x9D raises `UnicodeDecodeError` in the reader thread, which `_read_subprocess_output` (2588) swallows as `ValueError` — the reader exits, draining stops, the child blocks on a full pipe, and the item hangs until the 6-hour command timeout kills it. Invisible in development on UTF-8 locales. Fix: `encoding="utf-8", errors="replace"` on both calls. Confidence high
...
oze-plat-04 | open | med | organize_by_extension.py:967 — `_reusable_bucket_at_index` tests `filename not in names` against exact strings (`bucket_file_names`, 885), and `is_bucketed_file` (660) compares `ext_dir == resolve_real_extension(...)` case-sensitively. | platform: on case-insensitive filesystems (NTFS, APFS default) a bucket holding `readme.txt` accepts `README.txt` as non-colliding, then `os.link` raises `FileExistsError` on the same directory entry and `_reject_existing_target` skips the file — identically on every re-run, so it can never be organized. Likewise a pre-existing `PDF/p00000/x.pdf` fails the `ext_dir == "pdf"` test, is re-planned into the same physical directory, and self-collides into a permanent skip. No data loss (the no-clobber primitives hold) but the run is permanently incomplete on two of the three supported OSes. Fix: key membership and the ext-dir compare through `os.path.normcase`/`casefold` on case-insensitive hosts. Confidence high on mechanism, med on how often real trees hit it
oze-plat-05 | open | low | organize_by_extension.py:1437 — `_LINK_UNSUPPORTED_ERRNOS` lists `EPERM/ENOSYS/EOPNOTSUPP/EMLINK`, omitting the errnos Windows and macOS actually raise for "filesystem does not support hardlinks"; consumed at 1319 and 1558. | platform: on Windows, `os.link` on FAT32/exFAT — a USB stick or SD card, a core use case — fails `ERROR_INVALID_FUNCTION` (WinError 1), which CPython maps to `EINVAL`; on Darwin, msdos/exFAT/SMB return `ENOTSUP` (45), a *different constant* from `EOPNOTSUPP` (102) — they alias only on Linux, which is why the set looks complete there. Either way `move_file` re-raises, every file becomes a per-file skip, and the run organizes nothing and exits 3 instead of falling back to `_move_cross_device`, making the docstring promise at 1300 untrue off Linux. Fix: add `errno.ENOTSUP` and accept the Windows shape (`EINVAL` under `os.name == "nt"`, or `getattr(exc, "winerror", None) in (1, 50)`). Confidence high
rdv3-plat-02 | open | low | remove-deduplv3.py:175 — `_survivor` derives basename length with `os.sep`/`os.altsep`, i.e. the *host's* separators, but the paths come from an input record file whose separator convention was fixed by whichever machine generated the hash file. | platform: breaks the docstring's "same input always nominates the same survivor" determinism claim. A Linux-generated record holding a path with a literal backslash in the filename (legal on Linux: `/data/weird\name.txt`) yields basename `weird\name.txt` on Linux but `name.txt` on Windows, so a different survivor can win and the emitted script deletes a different file. Since the records and the emitted script are POSIX-dialect by design (settled rdv3-plat-01), split on `"/"` only. Confidence high on mechanism, low practical severity — either survivor is content-identical, but the stated guarantee is violated
rdv3-plat-03 | open | med | remove-deduplv3.py:204 — the rdv3-rel-01 guard collapses only byte-identical path strings (`dict.fromkeys`), so two case-variant spellings of one physical file survive as a group of two and one of them is emitted for `rm` at 210. | platform: on macOS (APFS, case-insensitive by default) or Windows, `~/Photos/x.jpg` and `~/photos/x.jpg` are the same file; the generated script — which runs fine under macOS `sh` — deletes it while the "saved" path pointed at the same inode. Silent loss of a unique file, in a script whose entire job is nominating deletions. On Linux the same input really is two files and the behaviour is correct, so the divergence is purely filesystem case sensitivity. Fix: within a group, detect `casefold()`-equal paths (or `os.path.samefile` when they exist locally) and warn instead of emitting `rm`. Confidence med — needs case-variant double-listing in the input, but the consequence is data loss
