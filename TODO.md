...
lq-perf-02 | open | med | link_queue.py:1429 — `_immediate_consumer` busy-polls with `time.sleep(0.05)` (20 Hz) under `_immediate_lock`; the immediate pool is spawned at startup, so those threads spin forever even when no immediate-mode protocol is used, and immediate items incur up to 50 ms latency. Block on a Condition/Event signalled by `_dispatch_immediate`. | always-on CPU proportional to worker_count
lq-perf-01 | open | med | link_queue.py:97 — `_yaml_emittable` trial-runs a full `yaml.dump` on every string field, and `_serialize_item` calls it for url/protocol/template/extra of every item on each `_save_state`; a 2000-item queue does ~8000 dump probes per debounced save. Fast-path plain printable-ASCII strings, trial-dump only the pathological case. | hot path = debounced state save; no fast-path/cache
