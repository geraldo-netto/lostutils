...
`--move-stall-seconds` sets the move stage's inactivity limit (default `300`, finite and positive). Byte progress during copying or recovery comparison, or a completed move, resets it. Cross-device copies run in reusable child processes so a stalled copy can be terminated; completed moves are drained before reporting totals. Native same-filesystem operations remain subject to OS I/O behavior. If a child cannot be reaped, the error identifies its PID and reports cancellation as unconfirmed.
...
Relocation refuses relative symlinks whose target path escapes the source tree, including dangling external targets. The check runs before creating destination directories and also applies to `--dry-run`, `--force`, and `--no-verify`. Change such links to absolute targets or keep their targets inside the source tree before retrying. Internal relative links and absolute links retain their literal targets.
...
Startup automatically resumes saved work, placing interrupted queued commands ahead of pending links. There is no review pause before execution; configure commands that tolerate repeating interrupted work.

Every command attempt has a 30-minute timeout by default and may be shortened with `command_timeout_seconds` or **Attempt timeout** in Settings → Dispatcher. Values above 1800 seconds are capped; zero, negative, and invalid saved values use the 30-minute default. A timeout terminates the command, with a short cleanup grace period. Queued links then use their remaining retry attempts and leave the queue when those attempts are exhausted; the default three attempts allow up to 90 minutes of command execution in total. Immediate commands still run only once.
