...
- Report contradictory requirements or instructions as soon as they are found. Automatically block the affected item and do not implement it until an explicit decision resolves the contradiction. Every contradiction — between instructions, docs, code, tests, or `TODO.md` items — gets its own `TODO.md` row in the "Blocked / Deferred" section (status `blocked`) naming both conflicting sources with `file:line` anchors and the decision needed; never report a contradiction only in chat. Delete the row once the decision is recorded and applied.
